...
NPM_DATABASE="$NPM_ROOT/data/database.sqlite"
NPM_PROXY_HOST="$NPM_ROOT/data/nginx/proxy_host/1.conf"
...
rtk proxy ssh "$REMOTE_HOST" "install -m 0600 '$NPM_DATABASE' '$backup/database.sqlite'"
rtk proxy ssh "$REMOTE_HOST" "install -m 0600 '$NPM_PROXY_HOST' '$backup/proxy-host-1.conf'"
...
rtk proxy ssh "$REMOTE_HOST" "python3 '$NPM_UPDATER_REMOTE' '$NPM_COMPOSE' '$NPM_DATABASE' '$NPM_PROXY_HOST'"
